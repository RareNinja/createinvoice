...
const PageInvoice = () => {
  ...
  const onSubmitItems = () => {
    ...
    const countryManufacture = document.getElementById(
      "countryManufacture"
    ) as any;
    ...
    const netWheightTotalUnit = document.getElementById(
      "netWheightTotalUnit"
    ) as any;
    ...
    const dataToSave = {
      ...
      netWheightTotal: netWheightTotal.value,
      netWheightTotalUnit: netWheightTotalUnit.value,
    };
  ...
  useEffect(() => {
    const totalpesoSum = faturaItensValues.reduce((acc, item) => {
      const value = item.netWheightTotal.toString().replace(/\,/g, ".");
      return acc + Number(value);
    }, 0);

    setPesoTotalValue(totalpesoSum);
  }, [faturaItensValues]);
  useEffect(() => {
    const totalUniPesoSum = faturaItensValues.reduce((acc, item) => {
      const value = item.netWheightTotalUnit.toString().replace(/\,/g, ".");
      return acc + Number(value);
    }, 0);

    setPesoUnitTotalValue(totalUniPesoSum);
  }, [faturaItensValues]);
  ...
  return (
    <div className="container max-w-100 mx-auto flex p-5 flex-col text-xs">
      ...
      <form
        ...
        className="bg-white p-5 gap-5 rounded-lg flex flex-row w-full overflow-hidden"
      >
        ...
        <div className="flex flex-col gap-5 w-1/2">
          <div className="flex flex-row gap-2 justify-end items-center h-5">
            <button
              type="button"
              onClick={() => {
                changeColorInvoice("bg-indigo-400");
              }}
              className={`rounded-md bg-indigo-400 w-5 h-5 transition-all hover:scale-110 ease-linear ${
                colorInvoice.includes("indigo") ? "w-6 h-6" : ""
              }`}
            ></button>
            <button
              type="button"
              onClick={() => {
                changeColorInvoice("bg-green-400");
              }}
              className={`rounded-md bg-green-400 w-5 h-5 transition-all hover:scale-110 ease-linear ${
                colorInvoice.includes("green") ? "w-6 h-6" : ""
              }`}
            ></button>
            <button
              type="button"
              onClick={() => {
                changeColorInvoice("bg-red-400");
              }}
              className={`rounded-md bg-red-400 w-5 h-5 transition-all hover:scale-110 ease-linear ${
                colorInvoice.includes("red") ? "w-6 h-6" : ""
              }`}
            ></button>
            <button
              type="button"
              onClick={() => {
                changeColorInvoice("bg-blue-400");
              }}
              className={`rounded-md bg-blue-400 w-5 h-5 transition-all hover:scale-110 ease-linear ${
                colorInvoice.includes("blue") ? "w-6 h-6" : ""
              }`}
            ></button>
            <button
              type="button"
              onClick={() => {
                changeColorInvoice("bg-orange-400");
              }}
              className={`rounded-md bg-orange-400 w-5 h-5 transition-all hover:scale-110 ease-linear ${
                colorInvoice.includes("orange") ? "w-6 h-6" : ""
              }`}
            ></button>
            <button
              type="button"
              className={`rounded-md bg-gray-400 w-5 h-5 ${
                colorInvoice.includes("gray") ? "w-6 h-6" : ""
              }`}
              onClick={() => {
                changeColorInvoice("bg-gray-400");
              }}
            ></button>
          </div>
          ...
          >
            ...
            <div className="flex flex-col gap-6 text-xs uppercase p-5 border bg-white">
              <div className="flex flex-col gap-4">
                <label className="text-lg mb-3 font-semibold">
                  Detalhes da Fatura
                </label>
              ...
              <div className="flex flex-col gap-4">
                <table>
                  ...
                  <tbody>
                    {faturaItensValues.length > 0 ? (
                      faturaItensValues.map((item: any) => {
                        return (
              ...
              <div className="flex flex-col">
                <div className="flex justify-between align-center">
                  <label className="font-semibold">
                    TOTAL {valuesCurrency[lang]} CFR ...
                  </label>
                  <p className="font-bold">
                    {totalValue.toLocaleString(lang, {
                      style: "currency",
                      currency: valuesCurrency[lang],
                    })}
                  </p>
                </div>
                <div className="flex justify-between align-center">
                  ...
                  <p className="font-bold">{pesototalValue}</p>
                </div>
                <div className="flex justify-between align-center">
                  <label className="font-semibold">
                    Peso Bruto Total (Kg) ...
                  </label>
                  <p className="font-bold">{pesoUnitTotalValue}</p>
                </div>
              </div>
                    ...
                    return (
                      <>
                        <div className="flex flex-row gap-5">
                          ...
                          <p>{dados.swiftCodeOne}</p>
                          <label className="font-semibold">
                            Account number:
                          </label>
                          <p>{dados.accountNumber}</p>
                        </div>
                        <div className="flex flex-row gap-5">
                          <label className="font-semibold">
                            Beneficiary Bank:
                          </label>
                          <p>{dados.beneficiaryBank}</p>
                          <label className="font-semibold">Swift Code:</label>
                          <p>{dados.swiftCodeTwe}</p>
                          <label className="font-semibold">
                            Beneficiary Customer:
                          </label>
                          <p>{dados.beneficiaryCustomer}</p>
                        </div>
                        <div className="flex flex-row gap-5">
